...
.. image:: https://travis-ci.org/mapbox/ufile.svg
   :target: https://travis-ci.org/mapbox/ufile

.. image:: https://coveralls.io/repos/mapbox/ufile/badge.png
   :target: https://coveralls.io/r/mapbox/ufile
...
.. code-block:: console
    ...
    git clone https://github.com/mapbox/ufile myproject
    cd myproject

Replace all occurrences of 'ufile' with the name of your own project.
(Note: the commands below require bash, find, and sed and are yet tested only on OS X.)

.. code-block:: console

    if [ -d ufile ]; then find . -not -path './.git*' -type f -exec sed -i '' -e 's/ufile/myproject/g' {} + ; fi
    mv ufile myproject
...
To help prevent uncustomized forks of ufile from being uploaded to PyPI,
...
https://www.mapbox.com/blog/ufile/.
